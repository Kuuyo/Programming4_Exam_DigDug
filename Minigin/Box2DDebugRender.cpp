...
namespace dae
{
	Box2DDebugRender::Box2DDebugRender(Renderer* pRenderer)
	{
		m_pRenderer = pRenderer->GetSDLRenderer();
	}

	SDL_Point Box2DDebugRender::ConvertB2Vec2ToSDLPoint(const b2Vec2 &vertex)
	{
		SDL_Point point;
	...
	std::vector<SDL_Point> Box2DDebugRender::ConvertB2Vec2ToSDLPoint(const b2Vec2* vertices, int32 vertexCount)
	{
	...
	void Box2DDebugRender::SetRenderColor(SDL_Renderer* renderer, const b2Color &color)
	{
		Uint8 r, g, b, a;
	...
	void Box2DDebugRender::DrawPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color &color)
	{
	...
	void Box2DDebugRender::DrawSolidPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color &color)
	{
		auto points = ConvertB2Vec2ToSDLPoint(vertices, vertexCount);
		points.push_back(points.at(0));
	...
	void Box2DDebugRender::DrawCircle(const b2Vec2& center, float32 radius, const b2Color& color)
	{
		SetRenderColor(m_pRenderer, color);
	...
	void Box2DDebugRender::DrawSolidCircle(const b2Vec2 &center, float32 radius, const b2Vec2&, const b2Color &color)
	{
		SetRenderColor(m_pRenderer, color);
	...
	void Box2DDebugRender::DrawSegment(const b2Vec2 &p1, const b2Vec2 &p2, const b2Color &color)
	{
	...
	void Box2DDebugRender::DrawTransform(const b2Transform &)
	{

	}

	void Box2DDebugRender::DrawPoint(const b2Vec2 &p, float32, const b2Color &color)
	{
